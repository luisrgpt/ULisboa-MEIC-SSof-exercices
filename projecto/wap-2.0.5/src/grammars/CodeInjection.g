...
/* Scanner to RFI, LFI, DT/PT, Source-code disclosure, OS Command, PHP code injection */


tree grammar CodeInjection; 
options { 
	tokenVocab=Php;
	backtrack=true;
	memoize = true;
...
definitionAttribute
    ...
    : ^(VAR ^(DOLLAR UnquotedString) value?)
    		{ attribName =  $UnquotedString.text; linha = $UnquotedString.line;}
...
paramDef // --Definicao de uma funcao -------------------------------------
    :	{
...
paramName returns [String nomeArg]// --Definicao de uma funcao -------------------------------------
    : ^(DOLLAR UnquotedString)
	{
	  try{
	    if (Flags[15] == 0){
		$nomeArg = $UnquotedString.text;
		//ArgFunctionSymbol afs = new ArgFunctionSymbol($UnquotedString.text, 0, $UnquotedString.line, currentFile, "var", null, posParmDefFunc);
    ...
    | ^(AMPERSAND ^(DOLLAR UnquotedString))
	{
	  try{
	    if (Flags[15] == 0){
		$nomeArg = $UnquotedString.text;
		//ArgFunctionSymbol afs = new ArgFunctionSymbol($UnquotedString.text, 0, $UnquotedString.line, currentFile, "pointer", null, posParmDefFunc);
...
expression
    ...
    | ^(INSTANCE_OF expression expression) {Flags[15] = 0;}
