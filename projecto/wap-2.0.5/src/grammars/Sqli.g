/*
 * Tree Grammar specification file for PHP 5.3 or higher
 * 
 ...
 * Ibéria Medeiros corrected and completed this tree grammar for PHP 5.3 or higher
 * 
...
expression
    ...
    | ^(EQUALS expression simpleRequire)
    	{
	  try{
	    if (Flags[15] == 0){
    ...
    | {Flags[0] = 1; /*assign_flag = 1;*/ } ^(EQUALS expression expression)       // --Atribuicao--------------
        {
		 try{       
	    if (Flags[15] == 0){
...
atom
    ...
    | DoubleQuotedString //atribuicao, call_function, include, function, return
	{
	  try{
	    if (Flags[15] == 0 && Flags[17] == 0){
...
variable // --definicao de variavel---- atribuicao, call_function, include ----------------------------------------------
    : ^(DOLLAR variable)    
	{
	  try{
	    if (Flags[15] == 0){
