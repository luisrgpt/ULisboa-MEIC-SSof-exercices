...
public class FuncAndIncLDAPI extends ScopedSymbol{

    // *** CONSTRUCTORS
    public FuncAndIncLDAPI(Scope scp) {
    ...
    public void resolveSymbolClassLdapi(MethodSymbol ms, Scope scp, TaintedTable mts, UntaintedTable mus, String filename, MethodSymbolCall mtsc, Instance inst, Map mct, Boolean ftime, Scope cs_att, Scope cs_acc, String typeMeth, Boolean isClassInstruction, Map varsDB) throws IOException{
    ...
    for (it = scp.getMembers().iterator(); it.hasNext();){
        ...
        switch (op){           
            case 1:{ // chamada de um metodo normal
                ...
                InstanceTable instTable = GlobalDataLDAPi.MainInstancesTable.get(fileInst);
                SymbolTable st = GlobalDataLDAPi.MainSymbolTable.get(fileInst);
                ...
                ClassTable ct = GlobalDataLDAPi.MainClassesTable.get(cf_name);
            ...
            case 2:{ // criacao de um objecto
                ftime = false;
                // criacao de um objecto
                if (sym.getName().equals("myldapi") == true)
                    ...
                    if (ms == null){
                        ...
                        instances_table = GlobalDataLDAPi.MainInstancesTable.get(file_inst);
                    ...
                    //Symbol sy = null;
                    for (Iterator <ClassTable> it3 = GlobalDataLDAPi.MainClassesTable.values().iterator(); it3.hasNext();){
                    ...
                    if (currentClass.getExtendsClassName() != null && currentClass.getExtendsClassName().equals("myldapi") == true){
                        varsDB.put("connectionDB", s);
                    ...
                    if (mss_final == null){ // a classe nao tem metodo construtor. logo fazer taint analise aos atributos
                        ...
                        MethodSymbolLDAPi meth_sym = new MethodSymbolLDAPi(mss_final);
                        int valueFinal = meth_sym.analyseMethodLdapi(mss_final,scp.getScopeSymbol().getCodeLine(), (Scope)sym, mts, mus, filename, mss_final_file, inst1, mct, typeMeth, null);
            ...
            case 4:{ // staticMember
                ...
                if (s.equals("self") == true){
                    ...
                    if (s.equals("parent") == true){
                        clas = inst.getInstanceClass();
                        clas_file = inst.getFileOfClass();
                        ClassTable ct = GlobalDataLDAPi.MainClassesTable.get(clas_file);
                ...
                if (clas.equals("") == false && sym.getIsClassMethod() == true){
                    ...
                    if (mss_final != null){
                        ...
                        MethodSymbolLDAPi meth_sym = new MethodSymbolLDAPi(mss_final);
                        int valueFinal = meth_sym.analyseMethodLdapi(mss_final,scp.getScopeSymbol().getCodeLine(), scp1, mts, mus, filename, mss_final_file, inst1, mct, typeMeth, varsDB);
            ...
            default:{
                ...
                if (sym.getRootScope() == null && (sym.getIsClassMethod() == false && sym.getIsUserFunction() == false)){
                    ...
                    if (GlobalDataLDAPi.LDAPConnectFunctions.contains(scp.getScopeSymbol().getName()) == true){
                        if (scp.getEnclosingScope() != null)
                            varsDB.put("connectionDB", scp.getEnclosingScope().getScopeName());
                        return;
                    }

                    if (GlobalDataLDAPi.LDAPSelectDbFunctions.contains(scp.getScopeSymbol().getName()) == true){
                        if (scp.getEnclosingScope() != null)
                    ...
                    if (scp.getScopeSymbol().getIsSecureFunction() == false){
                        // se o scope e' tainted e e' uma funcao LDAPI entao fazer correcao LDAP
                        if (scp.getScopeSymbol().getIsTaintedFunction() == true && scp.getScopeSymbol().getTainted() == 1 && scp.getScopeSymbol().getIsVulDetected() == false){
                            correctVulner(scp, filename, GlobalDataLDAPi.MainSymbolTable, null, ms, varsDB, mts, GlobalDataLDAPi.MainLinesToCorrect);
                        }
                    }
            }
            else{
                ...
                if (scp1.getMembers().size() > 0){
                    FuncAndIncLDAPI css2 = new FuncAndIncLDAPI(scp1);
                    css2.resolveSymbolClassLdapi(ms, scp1, mts, mus, filename, mtsc, inst, mct, ftime, csAtt_att, csAtt_acc, typeMeth, isClassInstruction, varsDB);
    ...
    private static Instance instanceMethod(MethodSymbol ms, Scope scp, String filename, String file_methClass, TaintedTable mts, UntaintedTable mus, Instance inst, MethodSymbolCall mtsc, Map mct, String typeMeth, Instance objRet, Map varsDB, InstanceTable instTab, SymbolTable symTab) throws IOException{
        ...
        if (enc.getScopeSymbol().getIsClassMethod() == false && objRet == null){
            ...
            if (s1 != null){
                ...
                if (mss_final != null){
                    MethodSymbolLDAPi meth_sym = new MethodSymbolLDAPi(mss_final);
                    int valueFinal = meth_sym.analyseMethodLdapi(mss_final,scp.getScopeSymbol().getCodeLine(), scp, mts, mus, filename, mss_final_file, inst, mct, typeMeth, varsDB);
                    scp.getScopeSymbol().setTainted(valueFinal);
            ...
            if (objRet != null){
                ...
                if (scp.getScopeSymbol().getIsClassMethod() == true){
                    ...
                    if (scp.getScopeSymbol().getRootScope() == null){                       
                        FuncAndIncLDAPI css2 = new FuncAndIncLDAPI(scp);
                        css2.resolveSymbolClassLdapi(null, scp, mts, mus, scp.getScopeSymbol().getFileSymbol(), mtsc, inst, mct, true, null, null, typeMeth, true, varsDB);
    ...
    public void resolveSymbolIncludeLDAPI(Scope scp, TaintedTable mts, UntaintedTable mus, Map mift, String filename, Map mft, Map mst, Map varsDB, Map mftt, Map MainLinesToCorrect) throws IOException{
        String filename_aux="";
...
try{
		...
		sy = it.next();
                if (sy.getIsVariableSymbol() == false){
                    ...
                    css.resolveSymbolIncludeLDAPI(cs, mts, mus, mift, nameIncFile, mft, mst, varsDB, mftt, MainLinesToCorrect);
                }
            }
} catch (Exception e){}
//} catch (Exception e){System.out.println("File: "+ nameIncFile+" not exist");}            
            ...
            if (scp.getScopeSymbol().getIsSecureFunction() == false){
                ...
                MethodSymbolLDAPi mss_final = null;
                    ...
                    if (mft.containsKey(filename) == true){
                        ...
                        for(Iterator <MethodSymbol> it = mt.getMembers().iterator(); it.hasNext();){
                            ...
                            if(mss.getFunctionName().equals(scp.getScopeName()) == true){
                                mss_final = new MethodSymbolLDAPi(mss);
                            ...
                            for(Iterator <String> it = st.getIncludeFiles().iterator(); it.hasNext();){
                                ...
                                if(st1.getHaveFunctions() == true){
                                    ...
                                    for(Iterator <MethodSymbol> it1 = mt.getMembers().iterator(); it1.hasNext();){
                                        mss = it1.next();
                                        if(mss.getFunctionName().equals(scp.getScopeName()) == true){
                                            //mss_final = (MethodSymbolLDAPi) mss;
                                            mss_final = new MethodSymbolLDAPi(mss);
                                            mss_final_file = s;
                ...
                if (mss_final != null){
                    String valueFinal = mss_final.analyseUserFunctionLdapi(mss, scp.getScopeSymbol().getCodeLine(), scp, mts, mus, filename, mss_final_file, mift, mft, mst, mftt, MainLinesToCorrect);
                ...
                Symbol sym1, sym;
                // verificar se o scope e' uma das funcoes de DB
                //if (scp.getScopeSymbol().getName().equals("myldap_connect") == true || scp.getScopeSymbol().getName().equals("myldapi_connect") == true){
                if (GlobalDataLDAPi.LDAPConnectFunctions.contains(scp.getScopeSymbol().getName()) == true){
                    if (scp.getEnclosingScope() != null)
                        varsDB.put("connectionDB", scp.getEnclosingScope().getScopeName());
                    return;
                ...
                //if (scp.getScopeSymbol().getName().equals("myldap_select_db") == true || scp.getScopeSymbol().getName().equals("myldapi_select_db") == true){
                if (GlobalDataLDAPi.LDAPSelectDbFunctions.contains(scp.getScopeSymbol().getName()) == true){
                    if (scp.getEnclosingScope() != null)
                        varsDB.put("selectDB", scp.getEnclosingScope().getScopeName());
                    return;
                ...
                if(scp.getScopeSymbol().getName().equals("myldapi_bind_param") == true){
                    Integer i = scp.getScopeSymbol().getCodeLine();
                    varsDB.put("lineLdapiBindParam", i.toString());
                    return;
                }
                ...
                for(;it.hasNext();){
                    ...
                    if (sym.getRootScope() == null){
                            ...
                            // se o scope e' tainted e e' uma funcao LDAPI entao fazer correcao LDAP
                            if ((scp.getScopeSymbol().getIsTaintedFunction() == true && scp.getScopeSymbol().getTainted() == 1 && scp.getScopeSymbol().getIsVulDetected() == false)
                                || scp.getScopeSymbol().getName().equals("myldapi_execute")){
                        ...
                        FuncAndIncLDAPI css1 = new FuncAndIncLDAPI(scp1);
                        css1.resolveSymbolIncludeLDAPI(sym.getRootScope(), mts, mus, mift, filename, mft, mst, varsDB, mftt, MainLinesToCorrect);
                        scp.resolveSymbol(scp, sym, mts, mus, filename);
                        mts.removeDuplicates(sym);
    ...
    // and on next times just need make data flow analysis to scope
    public void resolveSymbolFunctionLDAPI(MethodSymbol ms, Scope scp, TaintedTable mts, UntaintedTable mus, Map mift, String filename, Map mft, Map mst, Map varsDB, Map mftt, Map MainLinesToCorrect) throws IOException{
        String filename_aux="";
        if (scp.getScopeName().equals("include") == true){
            Symbol s = (Symbol) scp.getMembers().get(0);
            String nameIncFile = s.getName();
		...
		cs = (Scope) it.next();
                FuncAndIncLDAPI css = new FuncAndIncLDAPI(cs);
                css.resolveSymbolIncludeLDAPI(cs, mts, mus, mift, x, mft, mst, varsDB, mftt, MainLinesToCorrect);
            }
        }
        else{
            if (scp.getScopeSymbol().getIsSecureFunction() == false){
            ...
            for(;it.hasNext();){
                    ...
                    if (sym.getRootScope() == null){
                        ...
                        Scope scp1 = (Scope) sym.getRootScope();
                        FuncAndIncLDAPI css1 = new FuncAndIncLDAPI(scp1);
                        css1.resolveSymbolFunctionLDAPI(ms, sym.getRootScope(), mts, mus, mift, filename, mft, mst, varsDB, mftt, MainLinesToCorrect);
                        scp.resolveSymbol(scp, sym, mts, mus, filename);
                        mts.removeDuplicates(sym);
    ...
    public void resolveSymbolLDAPI(Scope scp, TaintedTable mts, UntaintedTable mus, Map mift, String filename, Map mft, Map mst, Map varsDB, Map mftt, Map MainLinesToCorrect, Map mct, Map mobjt) throws IOException{
        if (scp.getScopeName().equals("include")){            
            try{
            this.resolveSymbolIncludeLDAPI(scp, mts, mus, mift, filename, mft, mst, varsDB, mftt, MainLinesToCorrect);
            } catch (Exception e){
        ...
        if (scp.getScopeSymbol().getIsSecureFunction() == false){            
            if (scp.getScopeSymbol().getIsUserFunction() == true){               
                try{
                    this.resolveSymbolFunctionLDAPI(null, scp, mts, mus, mift, filename, mft, mst, varsDB, mftt, MainLinesToCorrect);
                } catch (Exception e){
            ...
            if(scp.getScopeSymbol().getIsClassInstruction() == true){
                try{
                this.resolveSymbolClassLdapi(null, scp, mts, mus, filename, null, null, mct, true, null, null, "", true, varsDB);
                } catch (Exception e){
            ...
            for(;it.hasNext();){
                ...
                if (sym.getRootScope() == null){
                    ...
                    Scope scp1 = (Scope) sym.getRootScope();
                    FuncAndIncLDAPI css1 = new FuncAndIncLDAPI(scp1);
                    css1.resolveSymbolLDAPI(sym.getRootScope(), mts, mus, mift, filename, mft, mst, varsDB, mftt, MainLinesToCorrect, mct, mobjt);
                    scp.resolveSymbol(scp, sym, mts, mus, filename);
                    mts.removeDuplicates(sym);
    ...
    private void correctVulner(Scope scp, String filename, Map mst, Map mift, MethodSymbol ms, Map varsDB, TaintedTable mts, Map MainLinesToCorrect) throws IOException{        
            ...
            VulnerabilityPathLDAPi vvp = new VulnerabilityPathLDAPi(scp);
            ...
            s = (String) varsDB.get("lineLdapiBindParam"); //This might be null!!
            int i = -1;
            try {
 ...
 * CORRECAO e DM
 */                        
            VulnerLDAPI vv = vvp.collectVulnPathLDAP(conn, db, i, GlobalDataLDAPi.UserInput, mts, currentSymbolTable, false, MainLinesToCorrect);
            ...
            */
            ...
            if (GlobalDataLDAPi.MainNumFP.containsKey(vv.getFileOf_SS()) == false)
                GlobalDataLDAPi.MainNumFP.put(vv.getFileOf_SS(), 0);
            ...
            if (vv.IsFP() == 1){                          
                ...
                GlobalDataLDAPi.MainNumFP.put(vv.getFileOf_SS(), nFP);
            }
            else{
                if (scp.getScopeSymbol().getTainted() == 1){                     
                    int nVul = GlobalDataLDAPi.MainNumVul.get(vv.getFileOf_SS()) + 1;
                    GlobalDataLDAPi.MainNumVul.put(vv.getFileOf_SS(), nVul);
                }
            ...
            if (scp.getScopeSymbol().getName().equals("myldapi_execute"))
                varsDB.put("lineLdapiBindParam", "-1");
           
            GlobalDataLDAPi.currentVulner = null;
        //}
    }
