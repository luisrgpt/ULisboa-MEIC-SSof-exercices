 ...
 * @author iberiam
 */
public class OutputAnalysisLDAPi {
    ...
    public static void outputAnalysis(String type_analyse, FileWriter outFile, String diff_date_ldapi, List files) throws IOException{
        int vuu = 0, fpp = 0;
        Integer j;
        ...
        if (vuu+fpp > 0){
            ...
            System.out.println("        - Number of vulnerable files: " +
                    setBoldText + GlobalDataLDAPi.MainListVulners.size() + setPlainText);
            ...
            for(Iterator <ListVulners> it = GlobalDataLDAPi.MainListVulners.values().iterator(); it.hasNext(); ){
        ...
        for (Iterator <ListVulners> it = GlobalDataLDAPi.MainListVulners.values().iterator(); it.hasNext();){
            ...
            if (GlobalDataApp.args_flags[5] == 0){
                ...
                if (GlobalDataLDAPi.MainSymbolTable.containsKey(file) == true)
                    ...
                    st = GlobalDataLDAPi.MainIncludeFilesTable.get(file);
                ...
                System.out.println("           - Real Vulnerabilities: " +
                        setBoldText + vuu + setPlainText);
                System.out.println("           - False positives: " +
                        setBoldText + fpp + setPlainText);
                if(GlobalDataApp.args_flags[4] == 1){
        ...
        if(GlobalDataApp.args_flags[0] == 0){
            ...
            LinesToCorrect ltc;
            int i = GlobalDataLDAPi.MainLinesToCorrect.size();
            for (Iterator <LinesToCorrect> it = GlobalDataLDAPi.MainLinesToCorrect.values().iterator(); it.hasNext();){
                ltc = it.next();
